defmodule ProcaWeb.ActionResolverTest do
  ...
  defp some_action_data(ap) do
    %{
      ...
      contact: %{
        first_name: "Blixa", last_name: "Bargeld",
        email: "ein@sturzen.de",
        address: %{
          postcode: "01-234", country: "pl"
        }
      },
      privacy: %{
        opt_in: true
  ...
  test "add petition action to a basic data campaign" do
    ...
    action_params = %{
      ...
      action: %{
        ...
        fields: [
          %{
            key: "medium", value: "tiktok"
          },
          %{
            key: "age", value: "44", transient: true
          }
        ]
      }
    }
  ...
  test "add signature with tracking" do
    ...
    params = some_action_data(ap) |> Map.put(:tracking, %{campaign: "email123", medium: "email", source: "team"})
    {:ok, created} = ProcaWeb.Resolvers.Action.add_action_contact(nil, params, nil)
    Proca.Server.Processing.sync()


    sup = Repo.get_by Supporter, fingerprint: Supporter.base_decode(created.contact_ref) |> elem(1) 
